...
const Insights = (props) => {
  ...
  async function getData(market, infotype, ticker) {
    await fetch(
      `http://localhost:8000/infodata/${market}/${infotype}/${ticker}`
    )
  ...
  async function getInsights(market, infotype, ticker) {
    await fetch(
      `http://localhost:8000/insights/${market}/${infotype}/${ticker}`
    )
      .then((response) => response.json())
      .then((info) => {
        setInsights(info);
  ...
  useEffect(() => {
    if (activeSubTab === "Data") {
      getData(props.market, activeTab, props.ticker);
    } else if (activeSubTab === "Insights") {
      setInsights("");
      if (activeTab !== "overall") {
        getInsights(props.market, activeTab, props.ticker);
      } else {
        getOpinion(props.market, props.ticker);
      }
    }
  }, [props.market, activeTab, props.ticker, activeSubTab]);
    ...
    <div className="flex flex-row basis-11/12 justify-center py-auto px-10">
      <div className="flex flex-col bg-white rounded-xl mx-auto p-4 justify-between space-y-1 shadow-sky-800 shadow-lg w-full ml-0 min-h-max mt-16 mb-10 place-items-center">
        <ul className="grid grid-flow-col bg-gray-100 rounded-full p-1 border text-sm font-semibold w-5/6">
          {tabList.map((tab) => (
            <li
              id={tab.replace(" ", "").toLowerCase()}
              onClick={() => setActiveTab(tab.replace(" ", "").toLowerCase())}
              className={`${
                activeTab === tab.replace(" ", "").toLowerCase()
    ...
    <div>
      {!disappear ? (
        <div
          id="bottom-banner"
          tabindex="-1"
          class="fixed bottom-0 left-0 z-50 flex justify-between w-full p-3 border-t border-gray-200 bg-gray-50 dark:bg-gray-700 dark:border-gray-600"
        >
          <div class="flex items-center mx-auto">
            <p class="flex items-center text-sm font-normal text-gray-500 dark:text-gray-400">
              ...
              <span>
                Not an investment advice. Not registered with any of the
                government bodies. Exercise caution before investing.
              </span>
            </p>
          </div>
          <div class="flex items-center">
            <button
              onClick={setDisappear(true)}
...
const Information = (props) => {
  return (
    <div
      className="flex-auto justify-center w-11/12 overflow-y-auto overflow-x-auto h-96 bg-slate-100 rounded-lg border"
      id="insight"
    >
