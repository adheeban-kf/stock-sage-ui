...
function TickerComponent(props) {
  ...
  async function getFilList(market, tickerSearch) {
    if (tickerSearch.length > 0) {
      console.log(market);
      await fetch(
        `http://localhost:8000/search/${activeCountry}/${tickerSearch}`
      )
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        className="bg-sky-700 text-white text-sm rounded-xl py-2 px-4 mt-4 w-1/3 focus:bg-sky-800 focus:ring-2 focus:ring-sky-500"
      >
        Submit to サ
      </button>
    </form>
...
export default function Ticker(props) {
  ...
  async function getClosePrice(market, ticker, range) {
    if (ticker !== "") {
      await fetch(
        `http://localhost:8000/closeprice/${market}/${ticker}?range=${range}`
      )
        .then((response) => response.json())
        .then((data) => {
          setGraphData(data);
  ...
  useEffect(() => {
    getClosePrice(market, ticker, range);
    props.setStateToApp(market, ticker, range);
  }, [market, ticker, range]);
