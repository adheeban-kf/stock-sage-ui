...
function Info(props) {
  ...
  var percentage = props.data["52WeekChange"]
    ? (props.data["52WeekChange"] * 100).toFixed(2)
    : NaN;
  ...
  const generateInfoList = (data) => {
    var dataList = Object.entries(data).map(([key, val]) => ({
      key: key,
      val: val,
    }));
      ...
      {Object.keys(data).length > 0 ? (
        <div className="flex flex-col bg-white rounded-xl mx-auto p-4 justify-between space-y-1 shadow-sky-800 shadow-lg w-5/6 ml-0 min-h-max">
          ...
          <div class="flex items-end my-6 space-x-2">
            <p class="text-2xl font-bold text-black">{data.longName}</p>
            {!percentageDecline ? (
        ...
        <div className="flex flex-col bg-white rounded-xl mx-auto p-4 justify-between space-y-1 shadow-sky-800 shadow-lg w-5/6 ml-0 h-4/6">
          <span className="text-black font-sans text-lg">
            Let's cook something! Enter a
            <span className="font-sans font-bold text-sky-800"> stock </span>of
            your choice and hit submit!
          </span>
        </div>
      )}
    </div>
